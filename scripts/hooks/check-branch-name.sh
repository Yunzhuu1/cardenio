...
if ! printf '%s\n' "$branch" | grep -E '^(feature|feat|bugfix|fix|hotfix|release|docs|chore)/[a-z0-9]+(\.[a-z0-9]+)*(-[a-z0-9]+(\.[a-z0-9]+)*)*$' >/dev/null 2>&1; then
  echo "Invalid branch name: $branch" >&2
  echo "Expected Conventional Branch format: <type>/<description>" >&2
  echo "Allowed types: feature, feat, bugfix, fix, hotfix, release, docs, chore" >&2
